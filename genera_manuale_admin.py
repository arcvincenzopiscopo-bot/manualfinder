...
class ManualePDF(FPDF):
    ...
    def table_row(self, cells, cols, fill=False):
        """cells: list of str; cols: list of (label, width)"""
        self.set_font("Helvetica", "", 9)
        self.set_text_color(30, 41, 59)
        self.set_fill_color(255, 255, 255) if not fill else self.set_fill_color(248, 250, 252)
...
pdf.body(
    "Il pannello admin di ManualFinder e' accessibile all'URL /#admin. "
...
pdf.body(
    ...
    "Non e' necessario riavviare il backend."
)
pdf.spacer()

pdf.h3("Struttura del pannello")
pdf.body("Il pannello e' organizzato in 11 tab accessibili dalla barra orizzontale in cima alla pagina:")
pdf.spacer(2)
...
pdf.spacer(6)
pdf.info_box(
    "Nessuna autenticazione e' richiesta per accedere al pannello. "
...
pdf.body(
    "Tabella con i dieci tipi macchina piu' cercati dagli ispettori. "
    "Per ogni tipo mostra il numero di utilizzi e i flag di patentino e verifiche periodiche."
)
pdf.spacer()

pdf.h3("Proposte stale")
pdf.body(
    "Se presenti, vengono elencate le proposte in attesa da piu' di 7 giorni "
...
pdf.spacer()
pdf.note(
    "Le proposte sono generate dall'analisi del testo OCR delle etichette fotografate "
    "dagli ispettori. Un sistema AI suggerisce automaticamente se la proposta e' "
    "simile a un tipo gia' esistente (campo 'Simile a')."
)
pdf.spacer()
...
pdf.spacer(2)
actions = [
    ("Salva come alias",    "La stringa proposta diventa un alias di un tipo esistente. "
                            "Selezionare il tipo dal dropdown. "
                            "Usare quando il tipo esiste gia' con nome diverso (es. 'muletto' -> 'carrello elevatore')."),
    ("Nuovo tipo canonico", "Crea un nuovo tipo nel catalogo. "
                            "Modificare il nome se necessario, impostare i flag patentino e verifiche. "
                            "Usare solo se il tipo e' genuinamente nuovo."),
    ("Rifiuta",             "Scarta la proposta. La stringa non viene registrata nel catalogo. "
...
pdf.bullet("Bordo arancione = proposta attiva da piu' di 7 giorni")
pdf.bullet("Bordo blu = proposta recente")
pdf.bullet("Riquadro giallo 'Simile a' = l'AI suggerisce un possibile merge con tipo esistente")
...
pdf.body(
    ...
    "sul server che non hanno ancora un tipo macchina associato nella tabella "
    "inail_manual_assignments."
)
pdf.spacer()

pdf.h3("Flusso di lavoro")
pdf.bullet("Aggiungi un nuovo PDF nella cartella 'pdf manuali/' del server")
pdf.bullet("Clicca 'Scansiona nuovi file su disco' — il sistema confronta i file presenti con le assegnazioni esistenti")
pdf.bullet("Per ogni proposta rilevata: modifica il nome canonico se necessario")
pdf.bullet("Clicca 'Approva' per creare un nuovo tipo canonico associato al file")
pdf.bullet("Clicca 'Rifiuta' per ignorare il file (non viene creato nessun tipo)")
pdf.spacer()

pdf.info_box(
    "La scansione e' idempotente: eseguirla piu' volte non crea duplicati. "
    "I file gia' assegnati non vengono riproposti."
)
pdf.spacer()
...
rows3 = [
    ...
    ("Nome finale",     "Campo modificabile: il nome canonico che verra' creato nel DB"),
]
...
pdf.body(
    "Gli alias permettono al sistema di riconoscere varianti testuali di uno stesso tipo macchina: "
...
pdf.bullet("Digitare il nuovo alias nel campo di testo (es. 'PLE', 'scissor lift', 'muletto')")
pdf.bullet("Premere Enter oppure cliccare '+ Aggiungi'")
pdf.spacer()

pdf.h3("Come eliminare un alias")
pdf.bullet("Cliccare l'icona cestino sulla riga dell'alias da rimuovere")
pdf.spacer()

pdf.h3("Badge 'source'")
pdf.body("Ogni alias mostra l'origine con un badge colorato:")
pdf.spacer(2)
cols4 = [("Source", 30), ("Significato", 142)]
pdf.table_header(cols4)
sources = [
    ("seed",  "Predefinito al primo avvio (da _SEED_ALIASES nel codice)"),
    ("admin", "Aggiunto manualmente dall'admin tramite questa tab"),
...
pdf.spacer()
pdf.note(
    "Gli alias sono case-insensitive e vengono normalizzati (spazi multipli -> spazio singolo, "
    "lowercase). Non e' necessario aggiungere varianti maiuscole/minuscole."
)
...
pdf.body(
    ...
    "Le modifiche si riflettono immediatamente nelle schede ispettive generate dal sistema."
)
pdf.spacer()

pdf.h3("Layout")
pdf.body(
    "La schermata e' divisa in due colonne: a sinistra la lista di tutti i tipi "
    "(filtrabile con la barra di ricerca), a destra l'editor per il tipo selezionato."
)
pdf.spacer()
...
rows5 = [
    ("Patentino / abilitazione", "Accordo Stato-Regioni 22/02/2012 — PLE, carrelli elevatori, gru, ecc."),
...
pdf.body(
    ...
    "Per associare un PDF locale usare la tab Manuali INAIL."
)
pdf.spacer()

pdf.h3("Vita utile stimata (anni)")
pdf.body(
    "Numero intero (es. 15). Viene mostrato nella scheda ispettiva come "
    "indicazione per l'ispettore sulla durata tipica dell'attrezzatura."
)
pdf.spacer()

pdf.h3("Hazard Intelligence")
pdf.body("Due campi per arricchire il contesto AI:")
pdf.bullet("Categoria INAIL (agente materiale): es. 'Apparecchi di sollevamento' — "
           "classificazione statistica INAIL usata per contestualizzare i rischi")
pdf.bullet("Focus rischi di categoria: 2-3 frasi sui rischi statisticamente piu' "
           "frequenti secondo le statistiche INAIL — iniettato nel prompt AI")
pdf.spacer()

pdf.h3("Bottoni AI globali (operano su tutti i tipi)")
pdf.spacer(2)
cols6 = [("Pulsante", 72), ("Azione", 100)]
pdf.table_header(cols6)
ai_btns = [
    ("Popola vita utile",         "Chiede all'AI di stimare la vita utile per tutti i tipi con campo NULL"),
    ("Popola hazard INAIL",       "Chiede all'AI di aggiornare categoria e focus rischi per tipi mancanti"),
    ("Associa quaderni INAIL",    "Associa automaticamente i PDF locali ai tipi con inail_search_hint NULL"),
]
for i, r in enumerate(ai_btns):
    pdf.table_row(list(r), cols6, fill=(i % 2 == 0))

pdf.note(
    "I bottoni AI operano solo sui campi NULL o non aggiornati di recente. "
    "Non sovrascrivono i valori gia' impostati manualmente."
)
...
pdf.body(
    "Mostra le scansioni recenti per cui non e' stato trovato un manuale specifico "
    "del produttore. Usata per richiedere al costruttore il manuale mancante."
)
pdf.spacer()
...
rows7 = [
    ("Foto",       "Miniatura dell'etichetta fotografata. Clic per ingrandire, icona graffetta per scaricare"),
    ...
    ("Fonte",      "Strategia usata per l'analisi (fallback_ai, pdf, inail, ecc.)"),
]
...
pdf.bullet("Invia ricerca — cerca automaticamente l'email del produttore e apre il client "
           "email con la richiesta di manuale precompilata (oggetto + corpo formali)")
pdf.bullet("X — nasconde la riga dalla lista senza cancellarla dal database")
pdf.spacer()

pdf.info_box(
    "Il sistema cerca automaticamente l'email del reparto assistenza tecnica del "
    "produttore. Se non trovata, il client email si apre con il campo 'A:' vuoto "
    "e il corpo precompilato con i dati della macchina."
)
...
pdf.body(
    "Archivio completo di tutte le analisi effettuate dagli ispettori (ultime 200). "
    "A differenza della tab Ricerche, qui sono visibili anche le analisi con manuale trovato."
)
pdf.spacer()

pdf.h3("Filtro per fonte")
pdf.body("Dropdown in alto per filtrare per tipo di fonte:")
pdf.spacer(2)
...
fonti = [
    ...
    ("inail",            "Solo quaderno INAIL (manuale produttore non trovato)"),
    ("fallback_ai",      "Nessun documento disponibile, solo inferenza AI"),
]
...
pdf.body(
    ...
    "per ispezionare i dettagli dell'etichetta originale. "
...
pdf.body(
    ...
    "produttore: lo integra con citazioni verificabili da normative e quaderni INAIL."
)
pdf.spacer()
...
pdf.bullet("Corpus attivo — verde se il corpus contiene almeno un documento")
pdf.spacer()

pdf.h3("Fonti nel corpus")
pdf.body(
    "Tabella dei documenti indicizzati con nome, tipo e filename. "
    "Il pulsante cestino rimuove un documento dal corpus."
)
pdf.spacer(2)
...
pdf.body(
    ...
    "Selezionare prima la categoria (Quaderno INAIL o Normativa), poi scegliere il file. "
    "Adatto per aggiornamenti rapidi di singoli documenti."
)
pdf.spacer()
pdf.note(
    "MiniLM e' un modello embedding leggero. Per qualita' ottimale indicizzare "
    "in locale con la GUI Python (python -m app.local_indexer) e caricare il DB pre-indicizzato."
)
pdf.spacer()
...
pdf.body(
    ...
    "Genera il file ZIP dalla GUI locale (pulsante 'Esporta ZIP ChromaDB'), "
    "poi caricalo qui. Sostituisce integralmente il corpus esistente su Render."
)
pdf.spacer()

pdf.h3("Re-indicizza tutto")
pdf.body(
    "Rielabora con MiniLM tutti i PDF presenti nel corpus. "
    "Da eseguire dopo l'upload di PDF singoli per sincronizzare l'indice. "
    "Puo' richiedere alcuni minuti."
)
...
pdf.body(
    ...
    "a un PDF fisicamente presente nella cartella 'pdf manuali/' del server. "
    "Questo PDF viene usato come fonte primaria per le schede ispettive "
    "(priorita' massima, affidabilita' 100%)."
)
pdf.spacer()

pdf.info_box(
    "Attenzione: questa tab gestisce i PDF fisici locali (quaderni INAIL scaricati). "
    "Non ha nessuna relazione con il corpus RAG (ChromaDB), che si gestisce "
    "nella tab 'Corpus RAG'."
)
pdf.spacer()

pdf.h3("Come aggiungere un'assegnazione")
pdf.bullet("Scegliere il Tipo macchina dal dropdown (lista di tutti i tipi canonici)")
pdf.bullet("Scegliere il PDF dal dropdown (mostra i file presenti in 'pdf manuali/')")
...
rows10 = [
    ...
    ("Su disco",      "Spunta verde = file trovato; triangolo arancione = file mancante"),
...
pdf.note(
    ...
    "predefinite. Se la tabella e' vuota ma i PDF sono in cartella, clicca 'Salva' "
    "per ogni assegnazione da ripristinare."
)
...
pdf.body(
    "Gestione delle normative tecniche associate a ogni tipo macchina "
    "(tabella machine_type_normative). Le normative vengono incluse nella "
    "sezione 'Normative applicabili' della scheda ispettiva."
)
pdf.spacer()

pdf.h3("Norma globale vs. specifica per tipo")
pdf.body(
    "Se il campo 'Tipo macchina' e' lasciato vuoto (selezionare 'Globale'), "
    "la norma appare in tutte le schede indipendentemente dal tipo macchina. "
    "Se si seleziona un tipo specifico, la norma appare solo per quel tipo."
)
pdf.spacer()

pdf.h3("Come aggiungere una norma")
pdf.bullet("Selezionare il tipo macchina (o lasciare vuoto per norma globale)")
pdf.bullet("Impostare il campo Ordine (numero intero, default 0) per controllare l'ordine di visualizzazione")
pdf.bullet("Inserire il testo della norma (es. 'EN ISO 3691-1:2015+A1:2020 — Carrelli industriali')")
pdf.bullet("Cliccare 'Aggiungi norma'")
pdf.spacer()

pdf.h3("Come eliminare una norma")
pdf.bullet("Cliccare il pulsante X (rosso) sulla riga della norma da rimuovere")
pdf.spacer()

pdf.note(
    "Al primo avvio il sistema esegue automaticamente il seed delle normative "
    "predefinite dalla tabella interna. Se la tabella e' vuota appare un messaggio "
    "informativo — riavviare il backend per ri-eseguire il seed."
)
...
pdf.body(
    "Gestisce i riferimenti normativi D.Lgs. 81/08 usati per le schede ispettive "
    "(sezione 'Riferimenti normativi' con testo degli articoli). "
    "I riferimenti sono pre-caricati dal seed e non possono essere aggiunti "
    "o eliminati da questa tab: solo l'associazione ai tipi macchina e' modificabile."
)
pdf.spacer()

pdf.h3("Globale vs. specifico")
pdf.body(
    "Un riferimento puo' essere applicato a tutti i tipi macchina (Globale) "
    "o limitato a un sottoinsieme specifico. "
    "I riferimenti globali vengono sempre inclusi in ogni scheda."
)
pdf.spacer()

pdf.h3("Come modificare l'associazione")
pdf.bullet("Cliccare il pulsante 'Modifica' sulla riga del riferimento")
pdf.bullet("Spuntare 'Globale' per applicarlo a tutti, oppure deselezionarlo")
pdf.bullet("Quando non e' Globale: selezionare/deselezionare i tipi macchina "
           "cliccando sulle chip colorate")
pdf.bullet("Cliccare 'Salva' per confermare")
pdf.spacer()

pdf.info_box(
    "Le chip mostrano i nomi dei tipi macchina (non gli ID numerici). "
...
scenari = [
    (
        ...
        [
            "Salvare il file PDF nella cartella 'pdf manuali/' del server",
            "Aprire la tab 'Manuali INAIL'",
            "Selezionare il tipo macchina e il PDF dal dropdown",
            "Inserire un titolo descrittivo (es. 'Scheda INAIL — Carrello elevatore')",
            "Cliccare 'Salva assegnazione'",
            "Verificare che 'Su disco' mostri la spunta verde",
        ]
    ),
    (
        ...
        [
            ...
            "Se e' un alias di un tipo esistente: cliccare 'Salva come alias' e selezionare il tipo",
            "Se e' un tipo genuinamente nuovo: cliccare 'Nuovo tipo canonico', "
            "modificare il nome, impostare i flag e confermare",
            "Se e' un errore OCR o un tipo accessorio: cliccare 'Rifiuta'",
        ]
    ),
    (
        ...
        [
            ...
            "Selezionare il tipo macchina dal dropdown",
            "Inserire il testo della norma e l'ordine di visualizzazione",
            "Cliccare 'Aggiungi norma'",
            "Se necessario, aggiornare anche i Riferimenti nella tab apposita",
        ]
    ),
    (
        "Aggiornare il corpus RAG con nuovi documenti",
        [
            "Indicizzare i PDF in locale con la GUI Python (python -m app.local_indexer)",
            "Esportare il database ChromaDB come ZIP",
            "Aprire la tab 'Corpus RAG'",
            "Cliccare 'Carica chroma_db.zip' e selezionare il file esportato",
